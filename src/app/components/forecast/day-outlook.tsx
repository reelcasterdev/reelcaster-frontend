...
export default function DayOutlook({ forecasts, selectedDay = 0, onDaySelect }: DayOutlookProps) {
  // Show all 14 days
  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700/50 shadow-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Best fishing periods for the next two weeks
          </p>
        </div>
      ...
      <div className="relative">
        ...
        <div 
          className="flex gap-3 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800/50 px-2"
          style={{
            scrollbarWidth: 'thin',
          ...
          {displayForecasts.map((forecast, index) => {
            ...
            return (
              <button
                ...
                className={`
                  group flex flex-col items-center min-w-[90px] p-4 rounded-2xl transition-all duration-300 transform hover:scale-105 relative
                  ${isSelected 
              ...
              >
                ...
                {/* Day Name */}
                <div className={`text-sm font-semibold mb-1 ${isSelected ? 'text-blue-300' : 'text-white group-hover:text-blue-200'} transition-colors`}>
                  {dayName}
                </div>
                
                {/* Date */}
                <div className={`text-xs mb-3 ${isSelected ? 'text-blue-200' : 'text-slate-400 group-hover:text-slate-300'} transition-colors`}>
                  {month} {dayDate}
                </div>
                ...
                <div className={`text-xs font-medium ${
                  ...
                  dayScore >= 8 ? 'text-green-300' :
                  dayScore >= 6 ? 'text-yellow-300' :
                  dayScore >= 4 ? 'text-orange-300' :
                  'text-red-300'
