...
function parseAreaNumber(label: string | null): number | null {
...
async function fetchNoticesForArea(area: number): Promise<Notice[]> {
...
export default async function SpotRegulationAlerts({
  dfoAreaLabel,
}: {
  dfoAreaLabel: string | null;
}) {
  const area = parseAreaNumber(dfoAreaLabel);
  if (area === null) return null;

  const notices = await fetchNoticesForArea(area);
  if (notices.length === 0) return null;

  return (
    <section
      data-testid="section-spot-regulation-alerts"
        ...
        <div className="flex items-end justify-between mb-5">
          <div>
            <p className="text-rc-text-muted text-xs tracking-[0.25em] uppercase font-medium mb-2 inline-flex items-center gap-2">
              ...
              DFO Pacific · {dfoAreaLabel ?? `Area ${area}`}
            </p>
          ...
          >
            All BC notices <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {notices.map((n) => (
            ...
            >
              ...
              <div className="mt-auto text-xs text-rc-text-muted">
